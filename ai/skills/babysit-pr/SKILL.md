...
6. **Address review comments** if any are open:
   ...
   - For each thread it **addressed**, post a concise reply noting what changed. For **AI bot** threads (Copilot, CodeRabbit, Cursor, Graphite, Gemini Code Assist, or any automated reviewer), also resolve the thread once the fix is committed and pushed (don't resolve before the push lands). For **human** threads, only reply — leave them open for the reviewer to resolve.
