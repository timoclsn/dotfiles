...
**A fired wake-up is not a user resume.** When the wake-up re-injects the loop prompt it arrives prefixed as `/loop /babysit-pr` (or `/loop /babysit-ci`), often with a "resuming /loop wakeup" marker — this is the loop continuing itself, **not** the user explicitly restarting. Treat it as a continuation round that is fully subject to the stop above: it must self-terminate, and it must **never** be read as an "explicit new start" that supersedes the stop. Only a **bare, user-typed `/babysit-ci` or `/babysit-pr`** (no `/loop` prefix, no wake-up marker) is an explicit resume that begins a new loop.
